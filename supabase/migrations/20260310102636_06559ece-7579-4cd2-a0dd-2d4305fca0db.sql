...
CREATE TABLE public.gym_classes (
  ...
  class_type TEXT NOT NULL DEFAULT 'basic',
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

ALTER TABLE public.gym_classes ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone authenticated can view classes" ON public.gym_classes FOR SELECT TO authenticated USING (true);
